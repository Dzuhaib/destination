...
export default function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const items = useMemo(() => (cart?.items || []).map((item) => ({ key: item.key, id: item.id, name: item.name, qty: item.quantity, price: amount(item.prices.price, item.prices.currency_minor_unit), image: item.images?.[0]?.src || "" })), [cart]);

  const value: CartContextValue = {
    items, count: cart?.items_count || 0,
    total: cart ? amount(cart.totals.total_price, cart.totals.currency_minor_unit) : 0,
    loading, error, refresh,
    addItem: async ({ id, quantity = 1 }) => mutate("/add-item", { id: Number(id), quantity }),
