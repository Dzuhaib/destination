  ...
  return <article className="group flex h-full flex-col border border-gray-200 bg-white">
    ...
    <div className="flex flex-1 flex-col p-5">
      ...
      <div className="mt-auto flex items-end justify-between gap-3 pt-6">
        ...
        {variable ? <Link href={`/product/${product.slug}`} className="border border-black px-3 py-2 text-xs font-bold uppercase">Choose options</Link> : <button type="button" title="Add to basket" disabled={adding || !product.purchasable || product.stockStatus === "out-of-stock"} onClick={async () => { setAdding(true); try { await addItem({ id: product.id }); } finally { setAdding(false); } }} className="grid h-10 w-10 place-items-center bg-black text-white transition-colors hover:bg-[#a6549e] disabled:cursor-not-allowed disabled:bg-gray-300"><ShoppingBag size={17} /><span className="sr-only">Add {product.name} to basket</span></button>}
      </div>
    </div>
  </article>;
}
